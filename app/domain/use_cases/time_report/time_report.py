...
class TimeReportUseCase:
    ...
    def create(self, type: str, user):
        ...
        last_time_report = self.time_report_repository.get_last_time_report(user)
        if last_time_report is not None and last_time_report.type == str(
            time_report_type
        ):
            raise ValueError("You can't set the same time report type twice in a roll")
        elif last_time_report is None and type != TimeReportType.IN.value:
            raise ValueError("You can't set a out time report without a in")
        self.time_report_repository.create(
            type=type, status=TimeReportStatus.APPROVED, user=user
        )
    ...
    def calculate_daily_report(self, daily_report_list: List[TimeReportDto]):
        ...
        for i in range(1, len(daily_report_list), 2):
            worked_time += (
                daily_report_list[i - 1].time - daily_report_list[i].time
            ).total_seconds()
        return {
            "entries": [asdict(report_entry) for report_entry in daily_report_list],
            "worked_time": f"{worked_time/3600} hours",
        }
    ...
    def calculate_monthly_report(self, monthly_report_list: List[TimeReportDto]):
        ...
        for i in range(1, len(monthly_report_list), 2):
            ...
            month_dict[day]["worked_time"] += (
                monthly_report_list[i - 1].time - monthly_report_list[i].time
            ).total_seconds()
            month_dict[day]["entries"].extend(
                [asdict(monthly_report_list[i - 1]), asdict(monthly_report_list[i])]
            )

        for day in month_dict.keys():
            month_dict[day][
                "worked_time"
            ] = f'{month_dict[day]["worked_time"]/3600} hours'
        return month_dict
